...
<template>
    <div class="min-h-screen bg-[#F8FAFC] flex text-[#334155] font-sans antialiased">
        ...
        <aside class="hidden lg:flex w-72 bg-white border-r border-[#E2E8F0] flex-col justify-between shadow-sm shrink-0 z-30 sticky top-0 h-screen overflow-y-auto">
            <div>
                <!-- Brand Header & Logo -->
                <div class="pt-7 pb-5 px-6 flex flex-col select-none border-b border-slate-100">
                    <Link :href="route('dashboard')" class="flex items-center gap-3">
                        <img v-if="appLogo" :src="appLogo" alt="Logo" class="w-9 h-9 object-contain shrink-0" />
                        <div v-else class="w-9 h-9 rounded-2xl bg-blue-600 text-white flex items-center justify-center font-black text-lg shadow-md shrink-0">
                            S
                        </div>
                ...
                <nav class="px-4 py-4 space-y-6">
                    ...
                    <div class="space-y-1 pb-6">
                        ...
                        <Link 
                            ...
                            class="group flex items-center gap-3.5 px-4 py-2.5 rounded-2xl text-xs transition duration-150"
                        >
                            <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>
            ...
            <div class="p-4 border-t border-slate-100 bg-slate-50/50">
                <div class="flex items-center justify-between p-2 bg-white rounded-2xl border border-slate-200/80 shadow-xs">
                    <div class="flex items-center gap-3 overflow-hidden">
                        <div class="w-8 h-8 rounded-xl bg-slate-900 text-white font-bold text-xs flex items-center justify-center shrink-0 uppercase">
                            {{ user.name.charAt(0) }}
                        </div>
                        <div class="flex flex-col overflow-hidden text-left">
                            <span class="text-xs font-bold text-slate-800 truncate">{{ user.name }}</span>
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider truncate">{{ user.role || 'Personel' }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <header class="bg-white border-b border-[#E2E8F0] sticky top-0 z-20 shadow-xs h-16 flex items-center px-4 sm:px-8 justify-between gap-4">
                
                <!-- Toggle Mobile & Search Bar -->
                <div class="flex items-center gap-3 flex-1">
                    <button @click="isMobileMenuOpen = !isMobileMenuOpen" class="lg:hidden p-2 rounded-xl text-slate-500 hover:bg-slate-100">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                    </button>

                    <!-- Global Search Bar (Ctrl + K) -->
                    <div class="max-w-md w-full relative flex items-center">
                        ...
                        />
                        <span class="hidden sm:inline-block absolute right-3 bg-white border border-slate-200 rounded-lg px-2 py-0.5 text-[9px] font-mono text-slate-400 font-bold shadow-xs">Ctrl K</span>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 shrink-0">
                    ...
                    <div class="relative">
                        ...
                        <div v-if="isNotifOpen" class="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 p-4 z-50 animate-float-card">
                    ...
                    <Dropdown align="right" width="48">
                        <template #trigger>
                            <button class="flex items-center gap-2.5 p-1.5 hover:bg-slate-100 rounded-2xl transition group">
                                ...
                                <svg class="w-4 h-4 text-slate-400 group-hover:text-slate-600 transition" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
                            </button>
                        </template>
                        <template #content>
