...
const submit = () => {
    form.post(route('skhpp.store'), {
        onSuccess: () => Swal.fire('BERHASIL', 'Permohonan SKHPP telah diterbitkan dan dikirim ke TTD Komandan.', 'success'),
    });
};
</script>

<template>
    <Head title="Terbitkan SKHPP Baru - SINDEN" />
        ...
        <div class="py-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between bg-white p-6 rounded-3xl shadow-xs border border-slate-100">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-xl font-black text-slate-900 uppercase tracking-tight">Formulir Penerbitan SKHPP</h1>
                        <p class="text-xs text-slate-500 font-medium">Input data permohonan Surat Keterangan Hasil Penelitian Personel</p>
                    </div>
                </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="bg-white p-6 rounded-3xl shadow-xs border border-slate-100 space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-slate-400 mt-1">Teks ini akan otomatis dimasukkan pada rincian Point 1c SKHPP.</p>
                    </div>
                </div>
                ...
                <!-- Point 2h: Anggota Pengikut (Multiple Members Checkbox) -->
                <div class="bg-white p-6 rounded-3xl shadow-xs border border-slate-100 space-y-4">
                ...
                <div class="bg-white p-6 rounded-3xl shadow-xs border border-slate-100 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea v-model="form.peruntukan" rows="3" required
                            placeholder="Contoh: Dalam rangka sebagai persyaratan mengikuti program pendidikan spesialis Bidang Ilmu Penyakit Dalam di Universitas Sebelas Maret (UNS) Solo"class="w-full text-xs font-medium px-4 py-3 rounded-2xl border-slate-200 focus:border-emerald-500 focus:ring-emerald-500"></textarea>
                    </div>
                </div>

                <!-- Unggahan Foto Pas Foto (3x4) -->
                <div class="bg-white p-6 rounded-3xl shadow-xs border border-slate-100 space-y-4">
                    <h2 class="text-xs font-black uppercase tracking-wider text-emerald-600 border-b pb-2">6. Unggahan Pas Foto (Format Resmi PDF)</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="file" @change="handleFoto1Change" accept="image/*" required class="block w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100" />
                        ...
                        <div v-if="form.is_pernikahan">
                            ...
                            <input type="file" @change="handleFoto2Change" accept="image/*" required class="block w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100" />
                ...
                <div class="flex justify-end gap-4 pt-4">
                    ...
                    <button type="submit" :disabled="form.processing"class="px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black uppercase tracking-wider rounded-2xl shadow-lg shadow-emerald-600/20 transition-all active:scale-95 disabled:opacity-50"> Kirim Permohonan SKHPP Ke Komandan
                    </button>
                </div>
            </form>
        </div>
