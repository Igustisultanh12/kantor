...
<template>
    ...
    <AuthenticatedLayout>
        <div class="space-y-8 font-sans">
            ...
            <!-- Hero Welcome Card (SISFOPERSKC Style) -->
            <div class="bg-white p-8 rounded-3xl border border-[#E2E8F0] shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <span class="text-slate-400 text-xs font-semibold">• Live System</span>
                    </div>
                    ...
                    <p class="text-xs text-slate-500 font-medium mt-1 leading-relaxed max-w-2xl">
                        Sistem Informasi Detasemen Intelijen siap mendukung efisiensi, pengawasan lokasi, dan akuntabilitas manajemen data personel & logistik hari ini.
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-white p-6 rounded-3xl border border-[#E2E8F0] shadow-xs flex items-center justify-between transition hover:shadow-md">
                    <div class="space-y-1">
                        <span class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest block">Buku Nomor Surat</span>
                        <h3 class="text-3xl font-black text-slate-900 leading-none py-1">{{ stats.total_logs }}</h3>
                        <p class="text-[11px] text-slate-500 font-medium">Total Registrasi Keluar</p>
                    </div>
                    <div class="w-14 h-14 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-lg shadow-xs">
                        📜
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-3xl border border-[#E2E8F0] shadow-xs flex items-center justify-between transition hover:shadow-md">
                    <div class="space-y-1">
                        <span class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest block">Arsip Surat</span>
                    ...
                    <div class="w-14 h-14 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-bold text-lg shadow-xs">
                        📁
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-3xl border border-[#E2E8F0] shadow-xs flex items-center justify-between transition hover:shadow-md">
                    <div class="space-y-1">
                        <span class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest block">Master Personnel</span>
                    ...
                    <div class="w-14 h-14 rounded-2xl bg-purple-50 text-purple-600 flex items-center justify-center font-bold text-lg shadow-xs">
                        👥
                    </div>
                </div>
                ...
                <div v-if="combined_activities" class="bg-white rounded-3xl border border-[#E2E8F0] p-6 lg:col-span-2 shadow-xs">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4 mb-5">
                        <h4 class="text-xs font-extrabold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                            ...
                            Log Aktivitas & Akses Terkini
                        </h4>
                    </div>
                    
                    <div v-if="combined_activities.length > 0" class="space-y-3">
                        <div v-for="activity in combined_activities" :key="activity.id" 
                             :class="activity.type === 'ADMIN_ACTION' ? 'bg-amber-50/50 border-amber-200/60' : 'bg-slate-50/70 border-slate-100'"
                             class="flex items-center justify-between p-4 rounded-2xl border transition hover:shadow-xs">
                            ...
                            <div class="min-w-0 flex-1 pr-4">
                                ...
                                <p :class="activity.type === 'ADMIN_ACTION' ? 'text-amber-800' : 'text-slate-600'" 
                ...
                <div v-if="pending_users" class="bg-white rounded-3xl border border-[#E2E8F0] p-6 shadow-xs">
                    ...
                    <div v-if="pending_users.length > 0" class="space-y-3">
                        <div v-for="pending in pending_users" :key="pending.id" class="flex items-center justify-between p-4 bg-slate-50/70 rounded-2xl border border-slate-100">
                            ...
                            <Link :href="route('users.index')" class="ms-3 px-3 py-1.5 bg-white border border-slate-200 hover:bg-slate-900 hover:text-white text-slate-700 rounded-xl shadow-xs transition-all text-[10px] font-extrabold uppercase tracking-wider">
                                Tinjau
                            </Link>
                        </div>
                    </div>
