...
// State Pemilihan Baris Transaksi untuk Cetak Laporan Rembes Khusus
const selectedCashIds = ref([]);

// State penampung kumpulan objek URL pratinjau nota
...
const handleReceiptChange = (e) => {
    ...
    receiptPreviews.value = files.map(file => ({
...
const handleEditReceiptChange = (e) => {
    ...
    editPreviews.value = files.map(file => ({
        url: URL.createObjectURL(file),
        is_pdf: file.type === 'application/pdf'
    }));
};
...
const submit = () => {
    form.post(route('technical-cash.store'), {
        forceFormData: true, 
        onSuccess: () => {
            form.reset();
            form.date = getTodayLocalDate();
            receiptPreviews.value = [];
            Swal.fire('Tersimpan', 'Log transaksi Dan Unit Teknis berhasil ditambahkan.', 'success');
        }
    });
};
...
const editTransaction = (cash) => {
    editForm.id = cash.id;
    editForm.date = cash.date ? cash.date.split('T')[0] : getTodayLocalDate();
    editForm.description = cash.description;
    editForm.debit = cash.debit;
    editForm.credit = cash.credit;
    editForm.receipt_files = [];
    editPreviews.value = cash.receipt_urls || [];
    showEditModal.value = true;
};

const submitEdit = () => {
    editForm.post(route('technical-cash.update', editForm.id), {
        forceFormData: true,
        onSuccess: () => {
            showEditModal.value = false;
            editForm.reset();
            editPreviews.value = [];
            Swal.fire('Tersimpan', 'Data transaksi Dan Unit Teknis berhasil dikalibrasi ulang.', 'success');
        }
    });
};
...
const deleteTransaction = (id) => {
    Swal.fire({
        ...
        text: "Log transaksi Dan Unit Teknis ini akan dihapus permanen!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            router.delete(route('technical-cash.destroy', id), {
...
<template>
    <Head title="Buku Kas Dan Unit Teknis" />
    <AuthenticatedLayout>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-sm border-collapse">
                                ...
                                >
                                    ...
                                    <td class="p-4 text-center bg-slate-100/50">
                                        <div class="flex justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button @click="editTransaction(cash)" class="p-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-600 hover:text-white transition shadow-sm cursor-pointer" title="Edit Transaksi">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                            </button>
                                            <button @click="deleteTransaction(cash.id)" class="p-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-600 hover:text-white transition shadow-sm cursor-pointer" title="Hapus Transaksi">
                            ...
                            <thead class="bg-slate-900 text-white font-black">
                                <tr>
                                    <td colspan="5" class="p-5 text-right uppercase italic text-xs tracking-widest border-r border-slate-800">Total Saldo Kas Dan Unit Teknis :</td>
                ...
                <template v-if="selectedCashIds.length > 0">
                    ...
                    <div style="margin-top: 15px; padding: 12px; border-left: 4px solid #000; background-color: #f9fafb; font-size: 11px; font-weight: bold; text-transform: uppercase;">
                        ...
                        <p style="margin: 6px 0 0 0; font-size: 9px; color: #555; font-style: italic; font-weight: normal;">
                            * Lampiran dokumen ini diterbitkan khusus untuk pengajuan klaim / penggantian dana reimbursement Dan Unit Teknis.
                        </p>
                    </div>
                </template>
            ...
            <div class="bg-white rounded-[2rem] shadow-2xl max-w-5xl w-full h-[90vh] overflow-hidden flex flex-col">
                <div class="p-6 border-b flex justify-between items-center bg-gray-50">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-black text-gray-800 uppercase text-xs tracking-widest">Pratinjau Kas Dan Unit Teknis ({{ selectedMonthName }})</h3>
                    </div>
            ...
            <div class="bg-white rounded-[2rem] shadow-2xl max-w-6xl w-full h-[85vh] overflow-hidden flex flex-col">
                <div class="p-6 border-b flex justify-between items-center bg-slate-900 text-white">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-black uppercase text-xs tracking-wider">Detail Transaksi Dan Unit Teknis & Berkas Lampiran</h3>
                    </div>
                ...
                <div class="flex-1 grid grid-cols-1 md:grid-cols-3 overflow-hidden bg-slate-50">
                    ...
                    <div class="md:col-span-2 p-6 overflow-y-auto bg-slate-100 flex flex-col justify-center items-center">
                        <div v-if="selectedTransaction?.receipt_urls && selectedTransaction.receipt_urls.length > 0" class="w-full space-y-6">
            ...
            <div class="bg-white rounded-[2rem] shadow-2xl max-w-lg w-full p-6 space-y-6">
                ...
                <form @submit.prevent="submitEdit" class="space-y-4 font-bold text-xs">
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t">
                        <button type="button" @click="showEditModal = false" class="px-4 py-2 text-xs uppercase text-slate-500">Batal</button>
                        <button type="submit" :disabled="editForm.processing" class="px-6 py-2.5 bg-blue-600 text-white rounded-xl text-xs uppercase tracking-wider font-extrabold hover:bg-blue-700">Simpan Perubahan</button>
                    </div>
                </form>
            </div>
        </div>
